...
const ESTADO_CONFIG: Record<string, EstadoConfig> = {
  ...
  APROBADA_FASE_1: {
    icon: CheckCircle2,
    color: "violet",
    titulo: "Fase 1 aprobada — Sube tu hoja de vida",
    mensaje:
      "Pasaste la validación automática. Ahora debes subir tu hoja de vida para avanzar a Fase 2.",
  },
  APROBADA_FASE_2: {
    icon: CheckCircle2,
    color: "violet",
...
export default async function DetalleConvocatoriaPage({ params }: Props) {
  ...
  if (!conv) notFound();
  if (conv.estado !== "PUBLICADA") redirect("/estudiante/mural");
  ...
  const mostrarFormulario =
    (!miPostulacion || miPostulacion.estado === "APROBADA_FASE_1") &&
    !fechaPasada;
                ...
                {conv.criterios.map((c) => (
                  ...
                  >
                    ...
                    <div>
                      ...
                      <p className="text-xs text-gray-500 mt-0.5">{describirCriterio(c)}</p>
                    </div>
                  </div>
                ))}
              </div>
                ...
                postulacionExistente={
                  miPostulacion
                    ? {
                        id: miPostulacion.id,
                        estado: miPostulacion.estado,
                        faseActual: miPostulacion.faseActual,
                        hojaDeVidaUrl: null,
                      }
                    : null
                }
              ...
              <div className="flex items-start gap-3 bg-gray-50 border border-gray-200 rounded-xl p-4">
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-500 mt-0.5">
                    La fecha de cierre fue el {formatearFechaLarga(conv.fechaFin)}.
                  </p>
                </div>
              </div>
            </div>
...
function EstadoPostulacionBox({ estado, fecha }: { estado: string; fecha: Date | string }) {
  const config = ESTADO_CONFIG[estado] ?? CONFIG_DEFAULT;
  const Icon = config.icon;
  const fechaStr = formatearFechaLarga(fecha);

  return (
    <div className={`flex items-start gap-3 rounded-xl p-4 border ${CONTAINER_COLORS[config.color]}`}>
      ...
      <div className="flex-1">
        ...
        <p className="text-[11px] mt-2 opacity-60">Te postulaste el {fechaStr}</p>
      </div>
    </div>
  );
}
...
function InfoItem({
  ...
  return (
    <div>
      ...
      <p className={`text-sm font-semibold ${highlight === "danger" ? "text-red-600" : "text-gray-900"}`}>
        {value}
      </p>
...
function describirCriterio(c: { tipo: string; campo: string; operador: string; valor: string }): string {
